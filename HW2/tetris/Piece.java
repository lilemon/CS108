...
public class Piece {
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint[] ret = flipVertical(body, height);
		ret = flipDiagnal(ret);
		return new Piece(ret);
	}
	
	private TPoint[] flipDiagnal(TPoint[] points) {
		TPoint[] ret = new TPoint[points.length];
		for (int i = 0; i < points.length; i++) {
			ret[i] = new TPoint(points[i].y, points[i].x);
		}
		return ret;
	}
	
	private TPoint[] flipVertical(TPoint[] points, int height) {
		TPoint[] ret = new TPoint[points.length];
		for (int i = 0; i < points.length; i++) {
			ret[i] = new TPoint(points[i].x, height - points[i].y - 1);
		}
		return ret;
	}
